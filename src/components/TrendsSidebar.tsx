...
export default function TrendsSidebar() {
  return (
    <div className="top- sticky-[5.25rem] hidden h-fit w-72 flex-none space-y-5 md:block lg:w-80">
...
const TrendingTopics = async function TrendingTopics() {
      ...
      {trendingTopics.map(({ hashtag, count }) => {
        ...
        return (
          <Link key="title" href={`/hashtag/${title}`} className="block">
            <p
              className="hover-underline line-clamp-1 break-all font-semibold"
              title={hashtag}
            >
              {hashtag}
            </p>
